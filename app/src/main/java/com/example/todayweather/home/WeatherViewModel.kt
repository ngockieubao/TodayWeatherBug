...
class WeatherViewModel(
    ...
    var listDataDaily = mutableListOf<HomeModel>()
    private val res = application.resources
    private val _properties = MutableLiveData<List<WeatherGetApi>>()

    // The external LiveData interface to the property is immutable, so only this class can modify
    val properties: LiveData<List<WeatherGetApi>>
        get() = _properties

//    private val _status = MutableLiveData<WeatherGetApi>()
//    val status: LiveData<WeatherGetApi>
//        get() = _status

    private fun getWeatherProperties() {

        viewModelScope.launch {
//            _status.value = WeatherGetApi.LOADING
            try {
//                val listResult =
                _properties.value = listOf(WeatherApi.retrofitService.getProperties())
//                _status.value = WeatherGetApi.DONE
//                Log.i("api", ${listResult})
            } catch (e: Exception) {
//                _status.value = WeatherGetApi.ERROR
//                _properties.value = ArrayList()
    ...
    private fun addDataDetail() {
        val index1 = HomeModel(1, "Cảm Giác Như", res.getString(R.string.humidity, 87))
        listDataDetail.add(index1)
        val index2 = HomeModel(2, "Độ Ẩm", res.getString(R.string.humidity, 62))
        listDataDetail.add(index2)
        val index3 = HomeModel(3, "Chỉ số UV", (11).toString())
        listDataDetail.add(index3)
        val index4 = HomeModel(4, "Tầm Nhìn", res.getString(R.string.visibility, 10))
        listDataDetail.add(index4)
        val index5 = HomeModel(5, "Điểm Sương", res.getString(R.string.dew_point, 24))
        listDataDetail.add(index5)
        val index6 = HomeModel(6, "Áp Suất", res.getString(R.string.pressure, 1007))
        listDataDetail.add(index6)
    }
    ...
    private suspend fun insert(detail: WeatherGetApi) {
